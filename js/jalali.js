...
JalaliDate1.jalaliToGregorian = function(j_y, j_m, j_d)
{
	j_y = parseInt(j_y);
	j_m = parseInt(j_m);
	j_d = parseInt(j_d);
	...
	for (var i=0; i < jm; ++i) j_day_no += JalaliDate1.j_days_in_month[i];
	...
	for (var i = 0; g_day_no >= JalaliDate1.g_days_in_month[i] + (i == 1 && leap); i++)
		g_day_no -= JalaliDate1.g_days_in_month[i] + (i == 1 && leap);
	var gm = i+1;
	var gd = g_day_no+1;
...
JalaliDate1.checkDate = function(j_y, j_m, j_d)
{
	return !(j_y < 0 || j_y > 32767 || j_m < 1 || j_m > 12 || j_d < 1 || j_d >
		(JalaliDate1.j_days_in_month[j_m-1] + (j_m == 12 && !((j_y-979)%33%4))));
}

JalaliDate1.gregorianToJalali = function(g_y, g_m, g_d)
{
	g_y = parseInt(g_y);
	g_m = parseInt(g_m);
	g_d = parseInt(g_d);
	...
	for (var i=0; i < gm; ++i)
	g_day_no += JalaliDate1.g_days_in_month[i];
	...
	for (var i = 0; i < 11 && j_day_no >= JalaliDate1.j_days_in_month[i]; ++i) {
		j_day_no -= JalaliDate1.j_days_in_month[i];
	}
	var jm = i+1;
	var jd = j_day_no+1;
...
Date.prototype.setJalaliFullYear = function(y, m, d) {
	...
	var j = JalaliDate1.gregorianToJalali(gy, gm+1, gd);
	if (y < 100) y += 1300;
	j[0] = y;
	if (m != undefined) {
		if (m > 11) {
	...
	var g = JalaliDate1.jalaliToGregorian(j[0], j[1], j[2]);
...
Date.prototype.setJalaliDate1 = function(d) {
	var gd = this.getDate();
	var gm = this.getMonth();
	var gy = this.getFullYear();
	var j = JalaliDate1.gregorianToJalali(gy, gm+1, gd);
...
Date.prototype.getJalaliDate1 = function() {
	var gd = this.getDate();
	var gm = this.getMonth();
	var gy = this.getFullYear();
	var j = JalaliDate1.gregorianToJalali(gy, gm+1, gd);
